...
BEGIN
  ...
  IF p_assegnazione IS NOT NULL THEN
    ...
    END IF;
    IF NULLIF(trim(p_assegnazione->>'camera_id'), '') IS NULL THEN
      RAISE EXCEPTION 'camera_mancante: indicare la camera dell''assegnazione';
    END IF;
    IF NULLIF(trim(p_assegnazione->>'posto'), '') IS NULL THEN
      RAISE EXCEPTION 'posto_mancante: indicare il posto letto dell''assegnazione';
    END IF;
  END IF;

  -- Stato derivato dalla presenza dell'assegnazione, mai dal payload:
  -- senza posto letto la persona entra in lista d'attesa (in_attesa_posto),
  -- altrimenti v_studenti_stadio la classificherebbe come archiviato.
  v_stato := CASE WHEN p_assegnazione IS NOT NULL THEN 'accolta' ELSE 'in_attesa_posto' END;
  ...
  VALUES (
    v_studente_id, v_stato, 'inserimento_manuale', 'pre_screening',
  ...
  VALUES (
    v_candidatura_id, NULL, v_stato, auth.uid(),
  ...
  IF p_assegnazione IS NOT NULL THEN
    ...
    VALUES (
      ...
      (p_assegnazione->>'camera_id')::uuid,
      (p_assegnazione->>'posto')::integer,
