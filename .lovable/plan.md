...
- `src/lib/anagraficaFatturazione.ts`: funzione pura `rigaDestinazioneAnagrafica({ modalita, anagraficaStudenteId, anaCorrente })` → `{ azione: 'aggiorna' | 'crea', id: string | null }`, sullo stesso modello di `statoDopoAnnullamento` e `deveInviareEsito`; alimenta sia il conteggio sia il salvataggio. Test in `src/test/anagrafica-fatturazione.test.ts`, quattro casi, uno per esito.
- Verifiche: tipi, build e suite completa con i nuovi test; riporto l'esito.
