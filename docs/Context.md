...
- **Anteprima obbligatoria**: con `conferma: false` la funzione **non chiama Fatture in Cloud e non scrive nulla**: esegue le guardie, costruisce il payload e lo restituisce con gli id dei registri accompagnati dal nome leggibile. È il primo tempo del dialogo `src/components/admin/contratti/EmettiFatturaDialog.tsx`, aperto dall'azione "Emetti fattura" sulla riga dello scadenzario in `src/pages/admin/ContrattoPage.tsx`. Non è una modalità di prova esposta all'operatore: è il tempo di controllo prima della conferma.
