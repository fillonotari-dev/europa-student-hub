...
- **Payload** in `supabase/functions/_shared/fic-fattura.ts`, modulo **puro** condiviso con il frontend via alias `@shared` e fissato da `src/test/fic-fattura.test.ts`: `number` deliberatamente omesso (il progressivo lo assegna Fatture in Cloud), `vat: { id }` dal registro aliquote (**non** il valore percentuale), `payment_method: { id }` al primo livello del documento (**non** dentro `payments_list`), scadenza calcolata in UTC come data di emissione più `fic_giorni_scadenza`.
- **`TIPO_DOCUMENTO`** vale oggi `'proforma'`: documento non fiscale, che non consuma il numero del sezionale ed è cancellabile. **Con `'proforma'` non avviene alcuna scrittura locale**: nessuna riga in `fatture`, nessun collegamento del canone, il canone resta `da_fatturare`. La ragione è di merito e di irreversibilità: una proforma non fattura un mese, e `canoni_protect_fatturati` da `fatturato` ammette solo il passaggio a `incassato`, mentre `riporta_contratto_in_bozza` rifiuta un contratto con canoni fatturati. Per lo stesso motivo `e_invoice: true` è nel payload **solo** quando il tipo è `invoice`: su un documento non fiscale la richiesta può essere rifiutata. Ogni esito elenca i passi saltati e il dialogo li mostra all'operatore; portare la costante a `'invoice'` attiva insieme scritture locali e flag elettronico.
