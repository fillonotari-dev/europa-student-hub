...
**Scadenza posticipata.** Una mensilità scade il giorno indicato dal contratto (`contratti.giorno_scadenza`) nel **mese successivo** a quello di competenza: il canone di agosto con giorno 15 scade il 15 settembre. È l'unico punto in cui la regola è calcolata — `generaScadenzario` in `src/lib/scadenzario.ts`, fissata dai casi di `src/test/scadenzario.test.ts` (mesi consecutivi, passaggio d'anno dicembre → gennaio, giorno 28 su gennaio → 28 febbraio, contratto di un solo mese). `attiva_contratto` scrive `scadenza` verbatim dal payload e non ricalcola nulla; `canoni.scadenza` non ha alcun CHECK che la leghi alla competenza. Di conseguenza in `/admin/fatturazione` il selettore del mese parte sul **mese precedente** a quello corrente: il lotto che si emette a settembre è quello di agosto.
...
**Impostazioni di emissione** su `public.impostazioni` (tabella a riga singola, id = 1): `fic_numerazione` (default `/S`), `fic_giorni_scadenza` (30), `fic_giorno_emissione` (25), `fic_iban`, `fic_metodo_pagamento` (`bonifico`), più gli identificativi letti dai registri dell'azienda — `fic_metodo_pagamento_id`, `fic_vat_id`, `fic_vat_valore` (vedi §"Registri dell'azienda"), più `fic_emette_fatture` (booleano, default `false`: spento crea proforma, acceso fatture reali). Si leggono e si modificano nella sezione "Fatture in Cloud" di `/admin/impostazioni` (`src/components/admin/impostazioni/FattureInCloudSection.tsx`). La sigla di numerazione diventa **di sola lettura** se esiste almeno una riga in `fatture` con `stato = 'emessa'`: cambiarla dopo il primo documento romperebbe la continuità del registro fiscale.
...
- **Interfaccia**: sezione "Fatture in Cloud" in `/admin/impostazioni`, sotto Listini, con il comando "Verifica connessione", le impostazioni di emissione e la scelta dei registri (`src/components/admin/impostazioni/FattureInCloudSection.tsx`).

#### Registri dell'azienda: metodo di pagamento e aliquota IVA

- **Perché esistono**: sul documento, l'IVA di riga è `vat: { id: <id nel registro dell'azienda> }` e il metodo di pagamento è `payment_method: { id }` **al primo livello del documento** — non un valore percentuale e non un nome dentro `payments_list`. La guida ufficiale *Invoice creation* dichiara che l'`id` è l'unico parametro richiesto e che un id inesistente produce errore. Quegli id appartengono all'account e vanno **letti**, mai scritti a mano.
- **`fic-registri`** (`supabase/functions/fic-registri/index.ts`): edge function riservata agli amministratori (`verify_jwt = true` in `supabase/config.toml` più controllo `has_role` in apertura; **non** è nel perimetro pubblico del §7). Sola lettura: `GET /c/{company_id}/info/payment_methods` e `GET /c/{company_id}/info/vat_types`. Non scrive su nessuna tabella di dominio; registra entrambe le chiamate in `fic_log` con `operazione = 'leggi_registri'`.
- **`supabase/functions/_shared/fic-client.ts`**: impianto delle chiamate (retry sulle sole risposte di quota, quote residue, `estraiDiagnosticaFic` per i rifiuti 400/422). Lo usa **soltanto `fic-registri`**. `fic-sync-anagrafica` **non** è stata riscritta di proposito: è l'unico percorso verso Fatture in Cloud validato sul campo, e portarla su questo modulo nello stesso intervento renderebbe impossibile attribuire un'eventuale rottura. È duplicazione di impianto, non di dominio; l'unificazione è rinviata a un intervento dedicato, come dichiarato nel commento in testa al modulo.
- **Dove finiscono le scelte**: `public.impostazioni` (riga singola) ha `fic_metodo_pagamento_id integer`, `fic_vat_id integer` e `fic_vat_valore numeric` (migration additiva del 06/09/2026, sole `ADD COLUMN` nullable). `fic_metodo_pagamento` (testo) resta e conserva il nome leggibile del metodo scelto.
- **Guardie che l'emissione rispetta** (`fic-emetti-fattura`, vedi il sotto-paragrafo "Emissione della fattura di una mensilità"): l'emissione **si ferma** se `fic_metodo_pagamento_id` o `fic_vat_id` sono nulli, e **si ferma** se `canoni.aliquota_iva` è diversa da `fic_vat_valore`. La modalità prova da sola non protegge: `dry_run` non chiama Fatture in Cloud, quindi un payload con quei campi sbagliati passerebbe il collaudo e fallirebbe alla prima emissione reale. Il payload restituito in prova mostrerà gli id **insieme al nome leggibile**.
- **IBAN sul documento** (verifica documentale, non implementata): le coordinate bancarie **non** arrivano dal campo `fic_iban` delle impostazioni, che oggi è solo un promemoria interno. Sulla fattura elettronica l'IBAN è `ei_data.bank_iban`, che la guida dichiara compilato **automaticamente dal metodo di pagamento predefinito dell'account** se impostato; `payment_account` in `payments_list` è cosa diversa e serve solo quando il pagamento è già `paid`, per dire su quale conto è entrato il denaro. Se in fattura l'IBAN dovrà comparire, la strada è il metodo di pagamento configurato su Fatture in Cloud (o `ei_data.bank_iban` esplicito), non il nostro campo.

#### Emissione della fattura di una mensilità

- **`fic-emetti-fattura`** (`supabase/functions/fic-emetti-fattura/index.ts`): edge function riservata agli amministratori (`verify_jwt = true` in `supabase/config.toml` più controllo `has_role` in apertura; **non** è nel perimetro pubblico del §7). Riceve `{ canone_ids, conferma }`, elabora i canoni **uno alla volta** — un fallimento non ferma gli altri — e restituisce un esito per ciascuno. Registra ogni chiamata in `fic_log` con `operazione = 'emetti_fattura'` e la stessa diagnostica 400/422 della sync (`fic_error_message`, `fic_validation_result`, nomi dei campi valorizzati e mai i valori).
- **Anteprima obbligatoria**: con `conferma: false` la funzione **non chiama Fatture in Cloud e non scrive nulla**: esegue le guardie, costruisce il payload e lo restituisce con gli id dei registri accompagnati dal nome leggibile. Non è una modalità di prova esposta all'operatore: è il tempo di controllo prima della conferma.
- **Le sei guardie**: canone in stato `da_fatturare` e non già collegato a una fattura; contratto in stato `attivo`; anagrafica di fatturazione collegata (`fic_entity_id` valorizzato); nessun campo obbligatorio mancante secondo `campiMancantiPerFattura` del modulo condiviso; `fic_metodo_pagamento_id` e `fic_vat_id` presenti nelle impostazioni; `canoni.aliquota_iva` uguale a `fic_vat_valore`. Prima di emettere, l'anagrafica viene **risincronizzata** su Fatture in Cloud, così il documento non nasce con un intestatario superato.
- **Payload** in `supabase/functions/_shared/fic-fattura.ts`, modulo **puro** condiviso con il frontend via alias `@shared` e fissato da `src/test/fic-fattura.test.ts`: `number` deliberatamente omesso (il progressivo lo assegna Fatture in Cloud), `vat: { id }` dal registro aliquote (**non** il valore percentuale), `payment_method: { id }` al primo livello del documento (**non** dentro `payments_list`), e `payments_list[0].due_date` presa da **`canoni.scadenza`**, cioè la data concordata nel contratto e mostrata nello scadenzario. `fic_giorni_scadenza` è solo il **ripiego** per il canone senza scadenza; se la scadenza è **anteriore** alla data di emissione — arretrato fatturato in ritardo — si usa la data di emissione, per non spedire un documento già scaduto alla nascita. Con la scadenza posticipata questo ripiego scatta **solo sugli arretrati**: una mensilità emessa nel mese di competenza o in quello successivo scade dopo l'emissione e non lo attiva mai. La regola vive in `scadenzaDocumento` ed è fissata dai tre casi in `src/test/fic-fattura.test.ts`. Prima, il documento dichiarava una scadenza diversa da quella dello scadenzario per la stessa mensilità.
- **Il tipo del documento è un'impostazione, non una costante**: `impostazioni.fic_emette_fatture` (booleano, default `false`). Spento → `proforma`; acceso → `invoice`. `TIPO_DOCUMENTO` e `SCRITTURE_LOCALI_ATTIVE` **non esistono più** in `_shared/fic-fattura.ts`: restano `tipoDocumentoDa(emetteFatture)` e `scrittureLocaliAttive(tipo)`, e il tipo è un **argomento** di `costruisciPayloadFattura`, letto una volta sola da `fic-emetti-fattura` insieme alle altre impostazioni. Portare l'interruttore su acceso cambia insieme quattro cose: (1) `type: 'invoice'` sul documento, (2) `e_invoice: true` nel payload, (3) la riga in `fatture` (prima `in_invio`, poi `emessa` con numero, data, `ei_status` e url), (4) `canoni.fattura_id` valorizzato e `canoni.stato` a `fatturato`. Con `proforma` nessuna di queste avviene, e ogni esito elenca i passi saltati. La ragione è di merito e di irreversibilità: una proforma non fattura un mese, `canoni_protect_fatturati` da `fatturato` ammette solo `incassato`, e `riporta_contratto_in_bozza` rifiuta un contratto con canoni fatturati. Spegnere di nuovo l'interruttore cambia solo i documenti futuri e non annulla nulla di già emesso.
- **Il modo non è mai invisibile**: l'interruttore "Emetti fatture reali" sta in `/admin/impostazioni` con le conseguenze dichiarate accanto; `/admin/fatturazione` mostra in testa una fascia che dichiara il modo in vigore, e il dialogo di conferma adegua titolo, avviso e pulsante leggendo **la stessa impostazione** — mai un testo scritto a mano.
- **Mesi parzialmente coperti**: il rateo sui giorni non si calcola, quindi il primo e l'ultimo mese di un contratto sono addebitati interi anche quando sono parziali e vanno corretti a mano **prima** dell'emissione (dopo, `canoni_protect_fatturati` blocca l'importo). Ogni riga dell'elenco il cui mese non è interamente coperto dal periodo del contratto porta l'annotazione "mese parziale: 17 giorni su 30", calcolata da `coperturaMese` in `src/lib/coperturaMese.ts` (funzione pura, test in `src/test/copertura-mese.test.ts`), non inline nel componente. È un avviso, non un blocco: è l'unico presidio contro il fatturare un mese intero per diciassette giorni senza accorgersene.
- **L'emissione ha una porta sola**: la pagina `/admin/fatturazione` (`src/pages/admin/Fatturazione.tsx`, voce di sidebar sotto Contratti in `src/components/admin/AdminSidebar.tsx`, rotta protetta sotto `AdminLayout` in `src/App.tsx`). La scheda contratto **non emette più**: attivare e chiudere sono momenti della vita di quel contratto, fatturare è un processo mensile che attraversa tutti i contratti insieme. Lo scadenzario continua a mostrare numero e data della fattura, in sola lettura.
- **Un mese per volta**: selettore di competenza in testa, preselezionato sul mese corrente, e l'elenco contiene le mensilità `da_fatturare` dei contratti `attivo` con competenza in quel mese, e basta. I mesi futuri si vedono ed emettono scegliendo il mese: non esiste una sezione separata. Sopra l'elenco, un avviso compare **solo** se esistono mensilità `da_fatturare` di contratti attivi con competenza precedente al mese scelto: dice quante sono e porta con un clic al mese più vecchio. Gli arretrati sono l'unico modo in cui questa pagina può fare danno per omissione, e non vanno mescolati con il lavoro del mese.
- **Le guardie non sono riscritte lato interfaccia**: al caricamento la pagina chiama `fic-emetti-fattura` con `conferma: false` sui canoni dell'elenco e usa gli esiti per marcare ogni riga emettibile o bloccata, col motivo; le bloccate non sono selezionabili. Le stesse anteprime vengono **passate** a `EmettiFatturaDialog`, che non richiama più la funzione: se le calcolasse da sé, lista e dialogo potrebbero mostrare numeri diversi. Le guardie che contano restano quelle rivalutate dalla funzione alla conferma.
- **Gruppi da dieci alla conferma** (l'anteprima, che non chiama Fatture in Cloud, può salire a cinquanta): ogni canone costa due chiamate esterne, cinquanta canoni sarebbero cento chiamate in sequenza in una sola richiesta. Se un gruppo fallisce o va in timeout la pagina **non invita a riprovare**: ricarica l'elenco e dichiara che alcuni documenti potrebbero essere stati creati e vanno verificati nel pannello di riconciliazione. L'esito resta a schermo come riepilogo di riuscite e fallite, non come un toast che sparisce.
- **Riconciliazione e archivio**: la sezione di riconciliazione compare solo se non è vuota — fatture in `in_invio` oppure `emessa` senza mensilità collegata — ed è sola lettura. In fondo, l'archivio delle fatture emesse (studente, competenza, numero, sezionale, data, totale, stato dell'invio elettronico) risponde a "questa l'abbiamo già fatta?" senza aprire Fatture in Cloud.
- **La trasmissione allo SDI non la fa il gestionale**: resta un'azione manuale su Fatture in Cloud, ed è detto nel dialogo di conferma.
- **Stato di chiusura (07/09/2026)**: il ciclo di emissione è **completo e collaudato in modalità proforma** — proforma `550829694` creata su Fatture in Cloud, nessuna scrittura locale, come previsto. Il passaggio a fatture reali è una decisione operativa, non tecnica: si prende accendendo l'interruttore.
- **Debiti dichiarati, aperti di proposito**: (a) nessun componente tabella condiviso — le tabelle di Contratti e Fatturazione ripetono lo stesso impianto, e l'allineamento degli importi a destra si deciderà quando il componente comune verrà estratto; (b) `fic-sync-anagrafica` **non** usa ancora `_shared/fic-client.ts`, per non rendere inattribuibile un'eventuale rottura dell'unico percorso validato sul campo; (c) il pannello di riconciliazione è **sola lettura**: non offre azioni di risoluzione, la verifica passa da Fatture in Cloud; (d) nessuna email di trasmissione della fattura allo studente, e nessuna trasmissione allo SDI dal gestionale.
